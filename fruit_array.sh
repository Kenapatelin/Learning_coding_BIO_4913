...
#create varibale FRUIT that holds file name
FRUIT="fruit.txt"

#echo value of variable FRUIT
#echo $FRUIT

#cut column 1 from file, remove header and reddirect output to new variable
...
do 
	...
	my_fruit[$COUNTER]=$LINE      #populate the element wiith value of line
	#echo ${my_fruit[$COUNTER]}    #echo the value of the element in the array that I just po>
	let COUNTER=COUNTER+1         #increse the counter by 1
...
#extract data in columnn 2 from FRUIT
...
while [ $COUNTER -lt ${#yum_scale[@]} ] # while value of counter is > 
do
	if [ ${yum_scale[$COUNTER]} = "yummy" ]; then
		echo ${my_fruit[$COUNTER]} # echo the value of the fruit 
	else
		echo "This fruit is not yummy" # echo "This fruit is not yummy"
	fi
